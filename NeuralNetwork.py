# BUGS: 
# 1) Always coming out as 0.5 for prediction? Training not working
...
class Math:
	...
	def calc_Z(weights, bias, prevActivation):
		Z = np.dot(weights, prevActivation) + bias
		return Z

	@staticmethod
	def calc_A(Z, activationFunction, dA = 0, takeDerivative = False):
		A = Z
		if (Math.ActivationFunction(activationFunction) == Math.ActivationFunction.RELU):
			A = Math.calc_RELU(Z, dA, takeDerivative)
		elif(Math.ActivationFunction(activationFunction) == Math.ActivationFunction.SIGMOID):
			A = Math.calc_sigmoid(Z, dA, takeDerivative)
		elif(Math.ActivationFunction(activationFunction) == Math.ActivationFunction.TANH):
			A = Math.calc_tanH(Z, dA, takeDerivative)
		return A

	@staticmethod
	def calc_sigmoid(Z, dA = 0, takeDerivative = False):
	...
	@staticmethod
	def calc_tanH(Z, dA = 0, takeDerivative = False):
	...
	@staticmethod
	def calc_RELU(Z, dA = 0, takeDerivative = False):
		relu = np.maximum(Z, 0)
		if (takeDerivative == True):
...
class NeuralNetwork:
	...
	def forward_propagate(self):
		...
		for l in range(1, len(self._networkLayers)):
			...
			if (l == len(self._networkLayers) - 1):
				networkData["A" + str(l)] = Math.calc_A(self._networkData["Z" + str(l)], Math.ActivationFunction.SIGMOID.value)
				networkData["Z" + str(l) + ":activation"] = Math.ActivationFunction.SIGMOID.value
			else:
				networkData["A" + str(l)] = Math.calc_A(self._networkData["Z" + str(l)], Math.ActivationFunction.RELU.value)
	...
	def train_network(self, inputData, trueOutputData, learningRate, iterations):
		# Sanity check
		assert(type(inputData).__module__ == np.__name__), "Input Data needs to be a numpy array."
		assert(type(trueOutputData).__module__ == np.__name__), "Real Output Data needs to be a numpy array."
		assert(np.shape(inputData)[0] == self._networkLayers[0]), "Input Data (" + str(np.shape(inputData)[0]) + ") mismatch with first layer size (" + str(self._networkLayers[0]) + ")."
		assert(np.shape(trueOutputData)[0] == self._networkLayers[len(self._networkLayers)-1]), "Real Output Data mismatch with last layer size."
		assert(np.shape(trueOutputData)[1] == np.shape(inputData)[1]), "Amount of Real Output Data and Input Data examples mismatch."
	...
	def predict(self, inputData):
		# Ensure we're getting the right type and amount of inputs to check
		assert (type(inputData).__module__ == np.__name__), "Input Data needs to be a numpy array."
		assert (np.shape(inputData)[0] == self._networkLayers[0]), "Input Data mismatch with first layer size."
